...
void Server::zapros_personal_cor_from_2_user(string mes_suf)//Запрос личной переписки по 2 пользователям
{
    ...
    id_to = reply;
    reply.clear();
    zapros = "select id, id_from, id_to, date, text, status from message where (id_from = '" + id_from + "' and id_to = '" + id_to + "') or (id_from = '" + id_to + "' and id_to = '" + id_from + "');";
    zaprosMySQL(zapros);
}
